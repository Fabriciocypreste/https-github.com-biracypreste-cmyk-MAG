...
import { Loader, Tv, Play, BarChart2, Users, History, Shirt, AlertCircle } from 'lucide-react';
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20">
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-[#1E1E1E] rounded-xl border border-white/10 overflow-hidden">
                        ...
                        <a href="/admin/futebol/serie-a" className="block p-3 text-center text-xs font-bold text-red-500 hover:bg-white/5 transition-colors border-t border-white/10 uppercase tracking-wider">
                            Ver tabela completa
                        </a>
                    </div>
                </div>
...
const LineupsView = ({ homeTeam, awayTeam }: { homeTeam: any, awayTeam: any }) => {
    // Mock Formations
    const homeFormation = [1, 4, 3, 3]; // GK, DEF, MID, ATT
    const awayFormation = [1, 4, 4, 2];

    const renderPlayer = (team: 'home' | 'away', role: string, idx: number) => (
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="relative w-full aspect-[2/3] md:aspect-[16/9] bg-[#2c8f46] rounded-lg border-2 border-white/20 overflow-hidden shadow-inner">
                ...
                {/* Players Container (Vertical Pitch) */}
                <div className="absolute inset-0 py-8 flex flex-col justify-between">
                    ...
                    <div className="flex flex-col gap-4 h-[45%] justify-start pt-4">
                        <div className="flex justify-center">{renderPlayer('away', 'GK', 0)}</div>
                        <div className="flex justify-around px-12">{[1,2,3,4].map(i => renderPlayer('away', 'DEF', i))}</div>
                        <div className="flex justify-around px-8">{[1,2,3,4].map(i => renderPlayer('away', 'MID', i))}</div>
                        <div className="flex justify-around px-20">{[1,2].map(i => renderPlayer('away', 'FWD', i))}</div>
                    </div>

                    {/* Home Team (Bottom) */}
                    <div className="flex flex-col-reverse gap-4 h-[45%] justify-start pb-4">
                        <div className="flex justify-center">{renderPlayer('home', 'GK', 0)}</div>
                        <div className="flex justify-around px-12">{[1,2,3,4].map(i => renderPlayer('home', 'DEF', i))}</div>
                        <div className="flex justify-around px-16">{[1,2,3].map(i => renderPlayer('home', 'MID', i))}</div>
                        <div className="flex justify-around px-12">{[1,2,3].map(i => renderPlayer('home', 'FWD', i))}</div>
                    </div>
                </div>
            </div>
