...
export default function MatchDetails() {
  ...
  // Mock data for Head-to-Head section
  const mockHistory = [
    { date: '10/08/24', homeScore: 2, awayScore: 1 },
    { date: '05/03/24', homeScore: 1, awayScore: 1 },
    { date: '22/09/23', homeScore: 0, awayScore: 1 },
    { date: '15/04/23', homeScore: 3, awayScore: 2 },
    { date: '11/11/22', homeScore: 0, awayScore: 0 },
  ];

  const homeWins = mockHistory.filter(g => g.homeScore > g.awayScore).length;
  const awayWins = mockHistory.filter(g => g.awayScore > g.homeScore).length;
  const draws = mockHistory.filter(g => g.homeScore === g.awayScore).length;
      ...
      <div className="max-w-6xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-[#1E1E1E] rounded-xl border border-white/5 p-6">
                ...
                <div className="p-3 bg-black/30 rounded-lg text-center mb-6 border border-white/10">
                    <p className="text-gray-400 text-xs mb-2">Resumo dos últimos {mockHistory.length} jogos</p>
                    <div className="flex justify-center gap-6 font-bold text-lg">
                        <div className="flex flex-col items-center">
                            <span className="text-white">{homeWins}</span>
                            <span className="text-[10px] text-gray-500 uppercase">{match.homeTeam.name}</span>
                        </div>
                        <div className="flex flex-col items-center">
                            <span className="text-white">{draws}</span>
                            <span className="text-[10px] text-gray-500 uppercase">Empates</span>
                        </div>
                        <div className="flex flex-col items-center">
                            <span className="text-white">{awayWins}</span>
                            <span className="text-[10px] text-gray-500 uppercase">{match.awayTeam.name}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    {mockHistory.map((game, i) => {
                        ...
                        if (game.homeScore > game.awayScore) {
                            resultText = 'Casa Venceu';
                            resultColor = 'text-green-500';
                        } else if (game.awayScore > game.homeScore) {
                            resultText = 'Visitante Venceu';
                            resultColor = 'text-red-500';
                        }
                        return (
                            <div key={i} className="flex items-center justify-between p-3 bg-black/20 rounded border border-white/5 hover:bg-white/5 transition-colors">
                                <span className="text-xs text-gray-500 font-mono w-20">{game.date}</span>
                                <div className="flex items-center gap-4 text-sm font-bold flex-1 justify-center">
                                    <span className="text-right w-2/5 truncate">{match.homeTeam.name}</span>
                                    <span className="px-3 py-1 bg-gray-700 rounded font-mono text-white">{game.homeScore} - {game.awayScore}</span>
                                    <span className="text-left w-2/5 truncate text-gray-400">{match.awayTeam.name}</span>
                                </div>
                                <span className={`text-xs font-bold w-28 text-right ${resultColor}`}>{resultText}</span>
                            </div>
                        );
                    })}
                </div>
